//! M/M/c queue simulation example with warm-up deletion and replications.
//!
//! Usage:
//!   cargo run --example mmc

use rand::SeedableRng;
use rand_chacha::ChaCha8Rng;
use rand_distr::{Distribution, Exp};
use simrs::{
    math::queueing::mmc_metrics,
...
fn run_mmc(
    ...
    for i in 0..c {
        ...
        sim.process(format!("server-{i}"), move |ctx| {
            let q = q_srv.clone();
            Box::pin(async move {
                let mut rng = ChaCha8Rng::seed_from_u64(seed ^ (0xCAFE_0000 + i as u64));
...
fn main() {
    ...
    let theory = mmc_metrics(lambda, mu, c).expect("stable M/M/c");
    ...
    for rep in 0..replications {
        let seed = 42 + rep;
        let (n, mean_w, mean_s) = run_mmc(lambda, mu, c, run_length, warmup_time, seed);
        total_completed += n;
    ...
    println!("M/M/{c} Queue — λ={lambda:.2}, μ={mu:.2}, ρ={rho:.3}");
